...
// Build array of Mon–Fri dates starting from this Monday
function buildDays() {
  const days = [];
  const now = new Date();
  // Go back to Monday
...
function getWeekGroups(days) {
  ...
  let cur = null;
  days.forEach((d, i) => {
    const wk = `${d.getFullYear()}-W${Math.floor(i / 5)}`;
    const label = d.toLocaleDateString("default", { month: "short", day: "numeric" });
    if (!cur || cur.key !== wk) { cur = { key: wk, label, start: i, count: 1 }; groups.push(cur); }
    else cur.count++;
  });
  return groups;
}
...
export default function App() {
  ...
  const [form, setForm] = useState({ title: "", memberId: 1, startDay: 0, durationDays: 5, fromJira: false, dueDateIdx: null });
  ...
  const openAdd = (memberId, dayIdx) => {
    setEditItem(null);
    setForm({ title: "", memberId, startDay: dayIdx, durationDays: 5, fromJira: false, dueDateIdx: null });
    setShowModal(true);
  };
  const openEdit = (e, a) => {
    e.stopPropagation();
    setEditItem(a);
    setForm({ title: a.title, memberId: a.memberId, startDay: a.startDay, durationDays: a.durationDays, fromJira: a.fromJira, dueDateIdx: a.dueDateIdx ?? null });
    setShowModal(true);
  };
  const save = () => {
  ...
  const handleDrop = (e, memberId, dayIdx) => {
    e.preventDefault();
    if (!dragging || dragging.fromJira) return;
    const ns = Math.max(0, Math.min(NUM_DAYS - dragging.durationDays, dayIdx - dragOffset));
    setAssignments(p => p.map(a => a.id === dragging.id ? { ...a, startDay: ns, memberId } : a));
    setDragging(null);
  };
  ...
  const syncFromJira = async () => {
    ...
    try {
      const jql = encodeURIComponent('status not in ("Done","Backlog","Blocked") ORDER BY duedate ASC');
      ...
      const ja = (data.issues || []).map(issue => {
        ...
        if (duedate) {
          ...
          const idx = DAYS.findIndex(d => dateKey(d) === dateKey(dd));
          dueDateIdx = idx >= 0 ? idx : Math.max(0, Math.min(NUM_DAYS - 1, DAYS.findLastIndex(d => d <= dd) + 1));
        }
        return { id: `jira-${issue.id}`, title: summary, memberId: member.id, startDay: null, durationDays: null, fromJira: true, jiraKey: issue.key, status: issue.fields.status?.name, dueDateIdx };
      });
  ...
  return (
    <div style={{ fontFamily: "'Inter','Segoe UI',sans-serif", background: "#0D0F14", height: "100vh", display: "flex", flexDirection: "column", color: "#C9D1D9", overflow: "hidden" }}>
      ...
      {/* Top Bar */}
      <div style={{ background: "#161B22", borderBottom: "1px solid #21262D", padding: "0 20px", display: "flex", alignItems: "center", height: 48, flexShrink: 0 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
            ...
            <thead>
              ...
              <tr style={{ background: "#161B22" }}>
                ...
                {monthGroups.map(g => (
                  <th key={g.key} colSpan={g.count} style={{ borderBottom: "1px solid #21262D", borderRight: "1px solid #21262D", padding: "5px 8px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "#8B949E", letterSpacing: "0.05em", position: "sticky", top: 0, zIndex: 19, background: "#161B22" }}>{g.label.toUpperCase()}</th>
                ))}
              </tr>
              {/* Week row */}
              <tr style={{ background: "#161B22" }}>
                ...
                {weekGroups.map(g => (
                  <th key={g.key} colSpan={g.count} style={{ borderBottom: "1px solid #30363D", borderRight: "1px solid #21262D", padding: "3px 6px", textAlign: "left", fontSize: 10, fontWeight: 600, color: g.start === 0 ? "#6366F1" : "#484F58", background: g.start === 0 ? "#1A1F2E" : "#161B22", position: "sticky", top: 27, zIndex: 19 }}>{g.label}</th>
                ))}
              </tr>
              {/* Day row */}
              <tr style={{ background: "#0D0F14" }}>
                <th style={{ borderBottom: "2px solid #30363D", borderRight: "1px solid #21262D", position: "sticky", top: 52, zIndex: 20, background: "#0D0F14" }} />
                {DAYS.map((d, i) => {
                  const isToday = dateKey(d) === TODAY_KEY;
                  return (
                    <th key={i} style={{ borderBottom: "2px solid #30363D", borderRight: "1px solid #1A1F26", padding: "2px 0", textAlign: "center", fontSize: 9, color: isToday ? "#6366F1" : "#484F58", fontWeight: isToday ? 800 : 400, background: isToday ? "#1A1F2E" : "transparent", position: "sticky", top: 52, zIndex: 19 }}>
                      {isToday ? "•" : d.getDate()}
                    </th>
                  );
                })}
            ...
            <tbody>
              {TEAM_MEMBERS.map(member => {
                ...
                const totalDays = mTasks.filter(a => a.durationDays).reduce((s, a) => s + a.durationDays, 0);
                const pct = Math.min(100, Math.round((totalDays / (NUM_WEEKS * 5)) * 100));
                ...
                return [
                  <tr key={`hdr-${member.id}`} style={{ background: "#161B22", cursor: "pointer" }} onClick={() => setExpanded(p => ({ ...p, [member.id]: !p[member.id] }))}>
                    ...
                    {DAYS.map((d, i) => (
                      <td key={i} style={{ borderBottom: "1px solid #21262D", borderRight: "1px solid #1A1F26", background: dateKey(d) === TODAY_KEY ? "#1A1F2E22" : "transparent" }}
                        onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(e, member.id, i)} />
                    ))}
                  </tr>,
                    ...
                    <tr key={`empty-${member.id}`}>
                      ...
                      {DAYS.map((d, i) => (
                        <td key={i} style={{ borderBottom: "1px solid #21262D", borderRight: "1px solid #1A1F26", background: dateKey(d) === TODAY_KEY ? "#1A1F2E11" : "transparent", cursor: "crosshair" }}
                          onClick={() => openAdd(member.id, i)} onDragOver={e => e.preventDefault()} onDrop={e => handleDrop(e, member.id, i)} />
                      ))}
                    </tr>
                  ),
                  ...
                  isExpanded && mTasks.map(a => (
                    <tr key={`task-${a.id}`}>
                      <td style={{ borderBottom: "1px solid #21262D", borderRight: "1px solid #21262D", padding: "0 10px 0 30px", height: 30, position: "sticky", left: 0, zIndex: 10, background: "#0D0F14" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                          ...
                          <span style={{ fontSize: 10, color: "#8B949E", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 130 }}>{a.title}</span>
                        </div>
                      </td>
                      {DAYS.map((d, i) => {
                        const isDueDay = a.fromJira && a.dueDateIdx === i;
                        const isStart = !a.fromJira && a.startDay === i;
                        const span = isStart ? a.durationDays : 1;
                        ...
                        return (
                          <td key={i} colSpan={isStart ? span : 1}
                            style={{
                              ...
                              background: dateKey(d) === TODAY_KEY ? "#1A1F2E11" : "transparent",
                              padding: isStart || isDueDay ? "3px 2px" : 0,
                              cursor: isStart || isDueDay ? "pointer" : "crosshair",
                              position: "relative",
                            }}
                            onClick={isStart || isDueDay ? e => openEdit(e, a) : () => openAdd(member.id, i)}
                            onDragOver={e => e.preventDefault()}
                            onDrop={e => handleDrop(e, member.id, i)}
                          >
                            {isStart && (
                              <div draggable onDragStart={e => handleDragStart(e, a)} onDragEnd={() => setDragging(null)}
      ...
      {tooltip && (
        <div style={{ position: "fixed", left: tooltip.x + 14, top: tooltip.y - 80, background: "#1C2128", border: "1px solid #30363D", borderRadius: 8, padding: "10px 14px", fontSize: 12, pointerEvents: "none", zIndex: 1000, boxShadow: "0 8px 32px rgba(0,0,0,.6)", minWidth: 180 }}>
          ...
          {tooltip.a.durationDays && <div style={{ color: "#484F58", fontSize: 10, marginTop: 3 }}>{tooltip.a.durationDays} day{tooltip.a.durationDays > 1 ? "s" : ""} · starts {DAYS[tooltip.a.startDay]?.toLocaleDateString("default", { month: "short", day: "numeric" })}</div>}
          {tooltip.a.dueDateIdx != null && <div style={{ color: "#F59E0B", fontSize: 10, marginTop: 3 }}>Due {DAYS[tooltip.a.dueDateIdx]?.toLocaleDateString("default", { month: "short", day: "numeric" })}</div>}
          ...
          {tooltip.a.fromJira && tooltip.a.jiraKey && (
            <div style={{ color: "#3B82F6", fontSize: 10, marginTop: 4 }}>↗ {tooltip.a.jiraKey} · click to open</div>
          )}
        </div>
      )}
              ...
              {!form.fromJira && (
                <div style={{ display: "flex", gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <select value={form.startDay} onChange={e => setForm(f => ({ ...f, startDay: Number(e.target.value) }))} style={{ display: "block", width: "100%", marginTop: 6, background: "#0D1117", border: "1px solid #30363D", borderRadius: 6, padding: "8px 10px", color: "#F0F6FC", fontSize: 13, outline: "none", boxSizing: "border-box", cursor: "pointer" }}>
                      {DAYS.map((d, i) => <option key={i} value={i}>{d.toLocaleDateString("default", { weekday: "short", month: "short", day: "numeric" })}</option>)}
                    </select>
                  </div>
                  <div style={{ flex: 1 }}>
